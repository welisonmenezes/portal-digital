...
class PostDelete extends Component {
    ...
    deletePost = () => {
        ...
        const path = '/api/post/' + this.state.post_id;
        fetch(`${process.env.REACT_APP_BASE_URL}${path}`, {
            ...
            .then(data => {
                if (this._isMounted) {
                    window.scrollTo(0, 0);
                    if (data.id) {
                        this.setState({ successMessage: 'Post deletado com sucesso' });
                ...
                {!this.state.successMessage &&
                    <div className="row">
                        <div className="col-md-6">
                            <h4>Deseja realmente deletar o post {this.state.post_id}?</h4>
                            <hr />
                            {this.state.isLoading && <Spinner />}
                            {!this.state.isLoading &&
                                <div>
                                    <button type="button" className="btn btn-danger" onClick={(evt) => this.deletePost(evt)}>Excluir</button>
